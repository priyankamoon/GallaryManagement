...
from api.utils import import_string
...
#     """
#     generates the metadata for the sidebarnavbar
#     in frontend for all the models
#     of a particular app
#     :param request: the request object
#     :param app: the app name for which the models to be fetched
#     :return: the metadata for app with all models name
#     """
#     meta_list = []
#     meta = {}
#     meta['sectionName'] = APPMETA_CONST['sectionName']
#     meta['icon'] = APPMETA_CONST['icon']
#     meta['screens'] = []
#     in_models = apps.get_app_config(app).get_models()
#     model_list = []
#     all_groups = request.user.groups.all()
#     for model in in_models:
#         if request.user.groups.filter(
#                 permissions__codename=FORMAT_PERMISSION_CODENAME['view']
#                         .format(model._meta.model_name)).exists():
#             model_list.append(model)
#     model_list = sorted(set(model_list), key=lambda x: x._meta.model_name, reverse=False)
#     for model in model_list:
#         m_all = []
#         for group in all_groups:
#             all_permi = list(request.user.groups.get(name=group.name).permissions.filter(
#                 content_type__app_label=model._meta.app_label,
#                 content_type__model=model._meta.model_name).values('codename'))
#             m_all = m_all + all_permi
#         info = {'screen': str(model._meta.model_name).capitalize(),
#                 'type': "master", 'model': model._meta.model_name,
#                 'related_app': model._meta.app_label,
#                 'title': DISPLAY_MODELS[model._meta.model_name] if model._meta.model_name in DISPLAY_MODELS.keys() else model._meta.model_name,
#                 'permissions': m_all}
#         meta['screens'].append(info)
#     meta_list.append(meta)
#
#     for screen in EXTRA_USER_META.keys():
#         extra_meta = {'sectionName': screen,
#                       'icon': SECTION_ICONS[screen],
#                       'screens': EXTRA_USER_META[screen]
#                       }
#         meta_list.append(extra_meta)
#     return meta_list
...
#         meta['screens'].append(info)
#     meta_list.append(meta)
#     for screens in EXTRA_USER_META.keys():
#         extra_meta = {'sectionName': screens,
#                       'icon': SECTION_ICONS[screens],
#                       'screens': []#EXTRA_USER_META[screens]
#                       }
#         info = {}
#         for screen in EXTRA_USER_META[screens]:
#             if screen['type'] == 'subsection':
#                 if request.user.groups.filter(
#                         permissions__codename=FORMAT_PERMISSION_CODENAME['view']
#                                 .format(screen['screen'].lower())).exists():
#                     extra_meta['screens'].append(screen)
#             elif screen['type'] == 'model':
#                 model_obj = apps.get_model(screen['app'],screen['model'])
#                 if request.user.groups.filter(
#                         permissions__codename=FORMAT_PERMISSION_CODENAME['view']
...
#                         m_all = m_all + all_permi
#                     info['screen'] = screen['screen']
#                     info['type'] = screen['type']
#                     info['title'] = screen['title']
...
#         if request.user.groups.filter(
#                 permissions__codename=FORMAT_PERMISSION_CODENAME['view']
#                         .format(i['sectionName'].lower())).exists():
#             final_list.append(i)
#     return final_list
...
def determine_appmeta_metadata(request, app):
    ...
    for screen in meta2.keys():
        if screen == 'MASTERS':
            ...
            under_meta['screens'] = generate_master_screens(request, meta2['MASTERS'][0]['app'], all_groups)
...
def determine_formview_metadata(request, model):
    ...
    for group in grps:
        permission = list(request.user.groups.get(name=group.name).permissions.filter(
    ...
    if request.parser_context['kwargs']['model'] in WORKFLOW_MODEL_LIST:
        temp = {'show_section': to_show_approval_button(request, request.query_params.get('id'))}
        model.UI_Meta.ui_specs["formview"][0].update(temp)
